...
describe('boss-config', () => {
  ...
  it('overrides env values', () => {
    process.env.BOSS_foo = '100';
    process.env.BOSS_bar__foo = '200';
    process.env.BOSS_complex__colour = 'red';
    process.env.BOSS_complex__fruit = 'apple';
    process.env.BOSS_complex__nested__colour = 'purple';
    process.env.BOSS_complex__nested__nested__fruit = 'banana';
    const config = loadConfig(__dirname + '/fixtures/one');
  ...
  it('converts string representations of booleans to booleans', () => {
    process.env.BOSS_foo = 'TRUE';
    process.env.BOSS_bar = 'FALSE';
    process.env.BOSS_baz = 'true';
    process.env.BOSS_zoo = 'false';
  ...
  it('parses boss specific args with BOSS_ prefix', () => {
    setArgv('--boss_foo', '--BOSS_bar', '--BOSS_BAZ', '--BOSS_foo__bar');

    const config = loadConfig(__dirname + '/fixture/one');

    // --boss_foo
    expect(config.foo).toBeUndefined();
    expect(config.boss_foo).toEqual(true);

    // --BOSS_bar
    expect(config.boss_bar).toEqual(undefined);
    expect(config.bar).toEqual(true);

    // --BOSS_BAZ
    expect(config.BOSS_BAZ).toBeUndefined();
    expect(config.boss_baz).toBeUndefined();
    expect(config.BAZ).toEqual(true);
...
function removeBossKeysFromEnv() {
  Object.keys(process.env).forEach((envKey) => {
    if (envKey.includes('BOSS')) {
